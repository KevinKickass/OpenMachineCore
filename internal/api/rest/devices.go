...
func (s *Server) listDevices(c *gin.Context) {
	devices := s.lifecycle.DeviceManager().ListDevices()
...
func (s *Server) getDevice(c *gin.Context) {
	...
	device, exists := s.lifecycle.DeviceManager().GetDevice(deviceID)
	if !exists {
...
func (s *Server) createDevice(c *gin.Context) {
	...
	deviceID, err := s.lifecycle.Storage().SaveOrUpdateDeviceComposition(c.Request.Context(), comp)
	if err != nil {
	...
	device, err := s.lifecycle.DeviceManager().LoadDeviceFromComposition(comp, 2*time.Second)
	if err != nil {
	...
	// Start poller
	pollInterval := s.lifecycle.Config().Modbus.DefaultPollInterval
	if err := s.lifecycle.DeviceManager().StartPoller(device.ID, pollInterval); err != nil {
...
func (s *Server) deleteDevice(c *gin.Context) {
	...
	// Get device first
	device, exists := s.lifecycle.DeviceManager().GetDeviceByName(instanceID)
	if !exists {
		c.JSON(http.StatusNotFound, gin.H{"error": "device not found"})
		return
	}
	...
	if err := s.lifecycle.Storage().DeleteDevice(c.Request.Context(), instanceID); err != nil {
